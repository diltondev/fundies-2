...
class DailyReportRequest(ReportRequest):
    ...
    A corresponding `FifteenMinuteObservation` nested class is used to support returns. Make a request by calling `.send()`
    ...
    class FifteenMinuteObservation():
        ...
        def __init__(self, 
            ...
            if not isinstance(site_name, str) or site_name == "":
                raise ValueError("Cannot initialize FifteenMinuteObservation with no/empty name!")
            self._site_name = site_name

            if not isinstance(site_id, int) or site_id <= 0:
                raise ValueError("Cannot initialize FifteenMinuteObservation with no/<=0 site_id!")
            self._site_id = site_id

            if not isinstance(end_time, datetime) or end_time.timestamp() <= 0:
                raise ValueError("Cannot initialize FifteenMinuteObservation with no/<=0 end_time!")
            self._observation_end_datetime = end_time

            if not isinstance(average_speed, (int, float)) or average_speed < 0:
                raise ValueError("Cannot initialize FifteenMinuteObservation with no/<0 average_speed!")
            self.average_speed = average_speed

            if not isinstance(vehicle_count, int) or vehicle_count < 0:
                raise ValueError("Cannot initialize FifteenMinuteObservation with no/<0 vehicle_count!")
            vehicle_count = vehicle_count
            
        @classmethod
        def from_dict(cls, site_id: int, data: dict) -> "DailyReportRequest.FifteenMinuteObservation":
            """
            ...
            if "Time Period Ending" not in data:
                raise ValueError("Cannot construct `FifteenMinuteObservation` from dict without valid timestamp")
            time: list[int] = [int(n) for n in data["Time Period Ending"].split(":")] # Split "00:11:22" time into int array of [0, 11, 22] for [hour, minute, second]
            dt.hour = time[0]
            dt.minute = time[1]
            dt.second = time[2]
    ...
    @property
    @staticmethod
    def ENDPOINT() -> str: f'{ReportRequest.ENDPOINT}/daily'
    ...
    def date(self, new: datetime) -> None:
        ...
        self._date = new.replace(hour=0, minute=0, second=0, microsecond=0) # Ignore time and set to midnight of date
    ...
    def send(self) -> list[FifteenMinuteObservation]:
        ...
        data = res.json()
        if "Rows" not in data:
            raise ValueError("`Rows` not found in `DailyReportReportRequest.send()`. Required field to form data.")
        observation_list: list[DailyReportRequest.FifteenMinuteObservation] = []
        for observation in data["Rows"]:
            observation_list.append(DailyReportRequest.FifteenMinuteObservation.from_dict(observation))
